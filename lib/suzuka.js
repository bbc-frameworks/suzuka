                    ...
                    if (data.share && siblings.length > 0) {
                        ...
                        siblings.forEach(function (u) {
                            ...
                            request.connection.on('error', function (err) {
                                sys.error('could not connect to sibling server ' + u.hostname + ' - ' + err.message);
                            });
...
function ipAllowed (ip, network, mask) {
...
function ipsAllowed (ip, allows) {
    for (var i = 0, l = allows.length; i < l; i++) {
        if (ipAllowed(ip, allows[i][0], allows[i][1])) {
            return true;
        }
    }
    return false;
...
*/
            ...
            req.addListener('end', function () {
                if (restrict.length > 0 && ! ipsAllowed(req.connection.remoteAddress, restrict)) {
                    sys.error('ignoring data from: ' + req.connection.remoteAddress);
...
function resolveRestrictions (restrict, then) {
    ...
    for (var i = 0, l = restrict.length; i < l; i++) {
        var match = restrict[i].match(/^(\d+\.\d+\.\d+\.\d+)\/(\d+)$/);
        if (match) {
            newRestrict[i] = [ match[1], parseInt(match[2], 10) ];
            ...
            dns.lookup(restrict[i], function (err, address) {
                if (err) {
                    throw new Error('dns lookup error: ' + err);
                }
                newRestrict.push([ipToInt(address), createMask(32)]);
                if (newRestrict.length === l) {
                    then(newRestrict);
                }
            });
...
function main() {
    ...
    for (var i = 0, l = args.length; i < l; i++) {
        if (args[i].match(/^--help$/)) {
            sys.puts('usage: suzuka [ --port=LISTEN_PORT ] [ --siblings=URL,... ] [ --restrict=IP[/MASK],... ] [ --daemonise ] [ --staticroot=ROOT ]');
            process.exit(0);
        }
