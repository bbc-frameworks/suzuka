...
var iframeProxy    = fs.readFileSync(staticRoot + '/iframeproxy.html', 'utf8'),
    testHtml       = fs.readFileSync(staticRoot + '/test.html', 'utf8'),
    suzukaJs       = fs.readFileSync(staticRoot + '/suzuka.js', 'utf8');
//    socketIoClient = fs.readFileSync('/usr/share/socket.io/socket.io.js');
...
function parentMain(port, siblings, restrict, shouldDaemonise) {
...
function main() {
    ...
    for (var i = 0, l = args.length; i < l; i++) {
        if (args[i].match(/^--help$/)) {
            sys.puts('usage: suzuka [ --port=LISTEN_PORT ] [ --siblings=URL,... ] [ --restrict=IP,... ] [ --daemonise ]');
            process.exit(0);
        }
    ...
    if (master) {
        parentMain(port, siblings, restrict, daemonise);
    }
    else {
        childMain();
    }
