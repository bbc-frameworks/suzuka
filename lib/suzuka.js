...
function parentMain(port, siblings, restrict, shouldDaemonise, staticRoot) {
    ...
    process.nextTick(function () {
        ...
        for (var i = 0; i < 8; ++i) {
            child = startChild(listen_fd, restrict, staticRoot, function (raw) {
...
function startChild(listen_fd, restrict, staticRoot, onreceive) {
    var control = net_binding.socketpair(),
        tunnel = net_binding.socketpair();

    var child = child_process.spawn(
        process.argv[0],
        [process.argv[1], '--slave', '--staticroot=' + staticRoot],
...
function childMain(staticRoot) {
    ...
    stdin.addListener('fd', function (fd) {
        if (listen_fd === null) {
            ...
            startServer(listen_fd, tunnel, staticRoot);
        }
    });
...
function startServer(listen_fd, tunnel, staticRoot) {
    var num_clients = 0,
        restrict = [],
        server,
        socket,
        iframeProxy = fs.readFileSync(staticRoot + '/iframeproxy.html', 'utf8'),
        testHtml    = fs.readFileSync(staticRoot + '/test.html', 'utf8'),
...
function main() {
    ...
    for (var i = 0, l = siblings.length; i < l; i++) {
        siblings[i] = {
            ...
            'pathname' : '/publish/private'
        };
    }

    if (master) {
        parentMain(port, siblings, restrict, daemonise, staticRoot);
    }
    else {
        childMain(staticRoot);
    }
}
